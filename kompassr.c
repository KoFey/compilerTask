...
int FDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  if ( PRNMET == 'Y' )                            /*если псевдооп.DC помеч.,*/
   {
   char type = TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0];	  
  						   /*то:                     */
    if                                            /* если псевдооперация DC */
     (                                            /* определяет константу   */
      type =='F' || type == 'H'			  /* типа F, то выполнить   */
     )                                            /* следующее:             */
     {
	     int size = (type == 'F') ? 4 : 2;
      T_SYM[ITSYM].DLSYM = size;                     /*  уст.длину симв. =  4, */
      T_SYM[ITSYM].PRPER = 'R';                   /*  а,призн.перемест.='R' */
      if ( CHADR % size )                            /*  и, если CHADR не указ.*/
       {                                          /*  на границу слова, то: */
	CHADR = (CHADR /size + 1) * size;               /*   уст.CHADR на гр.сл. и*/
  ...
  else
  {
   ...
   if ( CHADR % size )                               /*и CHADR не кратен 4,то: */
    CHADR = (CHADR /size + 1) * size;                   /* установ.CHADR на гр.сл.*/
  }
  char type = TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0];
  CHADR += (type == 'F') ? 4 : 2;
...
int FDS()                                         /*подпр.обр.пс.опер.DS    */
 {
  ...
  else return 1;  // Неверный формат операнда
  if ( PRNMET == 'Y' )                            /*если псевдооп.DC помеч.,*/
   {                                              /*то:                     */
      T_SYM[ITSYM].DLSYM = size;                     /*  уст.длину симв. =  4, */
      T_SYM[ITSYM].PRPER = 'R';                   /*  а,призн.перемест.='R' */
  ...
  if ( ARG == 2 )                                 /*формирование поля OPER  */
   {
    ...
    if (is_data_mode) {
        memcpy(TXT.STR_TXT.OPER, (char*)&RX.OP_RX.B2D2, 2);  // используем отswab'ленное значение
    } else {
        memcpy(TXT.STR_TXT.OPER, RR.BUF_OP_RR , 2);          // обычная команда RR
    }
...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
 {
  ...
  char *op = TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0];
  int size = op == 'F' ? 4 : 2;
  if
    (                                             /* если операнд начинается*/
     op=='F' || op=='H' /* с комбинации F'        */
    )                                             /* то:                    */
  ...
  is_data_mode = 1;
  STXT (size);     
  is_data_mode = 0;  /*формирование TXT-карты  */
...
fflush(stdout);
  ...
  if ( isalpha ( (int) *METKA2 ) || (int) *METKA2 == 64 )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
...
printf(">>> SUSING: Установлен R%d как база, SMESH = 0x%X\n", NBASRG, T_BASR[NBASRG - 1].SMESH);
fflush(stdout);
...
int SRR()                                         /*подпр.обр.опер.RR-форм. */
 {
	 printf("SRR>>> Команда: %s %s\n", T_MOP[I3].MNCOP, TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND);
fflush(stdout);
  char *METKA;                                    /*набор                   */
  char *METKA1;                                   /*рабочих                 */
...
printf("METKA1: %s | METKA2: %s\n",METKA1, METKA2);
  if ( isalpha ( (int) *METKA1 ) || (int) *METKA1 == 64 )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *METKA1 */
  ...
  if ( isalpha ( (int) *METKA2 ) || (int)*METKA2 == 64 )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
	...
	for ( I=0; I<15; I++ )                    /*далее в цикле из всех   */
	 {  
printf("Пробуем рег %d: PRDOST=%c, SMESH=0x%X, ZNSYM=0x%X, DELTA=0x%X\n",
       I+1, T_BASR[I].PRDOST, T_BASR[I].SMESH, ZNSYM, ZNSYM - T_BASR[I].SMESH);
	...
	printf(">>> Выбран базовый регистр R%d, смещение=0x%X\n", NBASRG, DELTA);
  ...
  if ( strcmp ( &NFIL [ strlen ( NFIL )-3 ], "ass" ) )
...

   {
	...
	if ( !fread ( ASSTEXT [I1], 80, 1, fp ) )
	 {
		 // Удаляем все символы '@' из строки ASSTEXT[I1]
              for (int j = 0, k = 0; j < 80; j++) {
                 if (ASSTEXT[I1][j] == '@') continue;  // пропускаем '@'
                 ASSTEXT[I1][k++] = ASSTEXT[I1][j];    // копируем остальные символы
                 if (k >= 80) break;                   // не выходим за пределы строки
               }
               // Заполняем оставшееся место пробелами, если '@' было много
               for (int j = strlen((char*)ASSTEXT[I1]); j < 80; j++) {
                   ASSTEXT[I1][j] = ' ';
                }
	  if ( feof ( fp ) )
	   goto main1;
...
*/
 ...
 CONT2:
    printf("\n===== ОТЛАДКА: Таблица символов (T_SYM) =====\n");
for (int i = 0; i <= ITSYM; i++) {
    printf("SYM[%2d] = %-8s, ZNSYM = %04X, DLSYM = %d, PRPER = %c\n",
        i, T_SYM[i].IMSYM, T_SYM[i].ZNSYM, T_SYM[i].DLSYM, T_SYM[i].PRPER);
}
 ...
 T_POP[5].BXPROG = SUSING;
  ...
  for ( I1=0; I1 < DL_ASSTEXT; I1++ )             /*для карт с 1 по конечную*/
   {     					  /*                        */
    memcpy ( TEK_ISX_KARTA.BUFCARD , ASSTEXT [I1],/*ч-ть очередн.карту в буф*/
					     80 );/*                        */
//printf("\n-- 2-й ПРОХОД: метка=[%.8s], операция=[%.5s], операнд=[%.12s]\n",
//    TEK_ISX_KARTA.STRUCT_BUFCARD.METKA,
//   TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAC,
//    TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND);
...
CONT5:
printf("\n======= ОТЛАДКА: Список сформированных объектных карт =======\n");

for (int i = 0; i < ITCARD; i++) {
    unsigned char *card = OBJTEXT[i];

    // ESD-карта
    if (card[1] == 'E' && card[2] == 'S' && card[3] == 'D') {
        printf("Карта %2d: [ESD] Имя: %.8s\n", i+1, &card[14]);
    }

    // TXT-карта
    else if (card[1] == 'T' && card[2] == 'X' && card[3] == 'T') {
        int addr = (card[5] << 8) | card[6];
        int len  = card[10];
        printf("Карта %2d: [TXT] Адрес: %04X, Длина: %d байт, Первые байты: %02X %02X\n",
            i+1, addr, len, card[12], card[13]);
    }

    // END-карта
    else if (card[1] == 'E' && card[2] == 'N' && card[3] == 'D') {
        printf("Карта %2d: [END]\n", i+1);
    }

    // Неизвестный тип
    else {
        printf("Карта %2d: [???] Тип не распознан: %02X %02X %02X %02X\n",
            i+1, card[0], card[1], card[2], card[3]);
    }
}

printf("=============================================================\n");
printf("Всего сформировано %d карт (включая ESD, TXT, END)\n", ITCARD);
